        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.use_depth:
                    ...
                    n = max(args.train_batch_size - global_step//2000, 0)
                    if n == args.train_batch_size:
                        gt_image = depth.repeat(1, 3, 1, 1)
                    elif n == 0:
                        gt_image = im
                    else:
                        depth_indices = torch.randperm(args.train_batch_size)[:n]
                        gt_image = torch.clone(im)
                        gt_image[depth_indices] = depth[depth_indices].repeat(1, 3, 1, 1)
                else:
                    gt_image = im
